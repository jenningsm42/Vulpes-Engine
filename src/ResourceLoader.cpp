...
namespace vul {
    ...
    Handle<Mesh> ResourceLoader::loadMeshFromData(const MeshData& meshData) {
        ...
        if (!meshData.normals.empty()) vboCount++;
        if (!meshData.tangents.empty() && !meshData.bitangents.empty()) vboCount += 2;
        if (!meshData.UVCoordinates.empty()) vboCount++;
        if (!meshData.vertexWeights.empty() && !meshData.vertexBones.empty()) vboCount += 2;
        ...
        if (!meshData.tangents.empty() && !meshData.bitangents.empty()) {
            glBindBuffer(GL_ARRAY_BUFFER, vbo[vboIndex++]);
            ...
            glBindBuffer(GL_ARRAY_BUFFER, vbo[vboIndex++]);
            glBufferData(GL_ARRAY_BUFFER, meshData.bitangents.size() * sizeof(float), meshData.bitangents.data(), GL_STATIC_DRAW);
            glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, 0, 0);
            glEnableVertexAttribArray(4);
        }

        // UV Coordinates
        if (!meshData.UVCoordinates.empty()) {
            glBindBuffer(GL_ARRAY_BUFFER, vbo[vboIndex++]);
            glBufferData(GL_ARRAY_BUFFER, meshData.UVCoordinates.size() * sizeof(float), meshData.UVCoordinates.data(), GL_STATIC_DRAW);
            glVertexAttribPointer(4, 2, GL_FLOAT, GL_FALSE, 0, 0);
            glEnableVertexAttribArray(2);
        }

        // Vertex weights and bones indices
        if (!meshData.vertexWeights.empty() && !meshData.vertexBones.empty()) {
            glBindBuffer(GL_ARRAY_BUFFER, vbo[vboIndex++]);
            glBufferData(GL_ARRAY_BUFFER, meshData.vertexWeights.size() * sizeof(float), meshData.vertexWeights.data(), GL_STATIC_DRAW);
    ...
    void ResourceLoader::createPlane() {
        MeshData meshData;
        meshData.resize(4, 6, true, true, true);
    ...
    void ResourceLoader::createSphere() {
        ...
        MeshData meshData;
        meshData.resize(resolution * (resolution + 1), ((resolution - 1) * resolution * 2) * 3, true, true, true);
    ...
    void ResourceLoader::prefilterCubeMap(Handle<Texture>& texture, uint32_t width) {
        ...
        uint32_t numMipMaps = static_cast<uint32_t>(std::log2(width));
